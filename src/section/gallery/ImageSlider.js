...
const ImageSlider = (props) => {
  ...
  const movedHandler = (splider) => {
    setCurrentIndex(splider.current.splide.index + 1);
  }

  return (
    <Modal onClose={props.onClose}>
      <div className={classes['slider']}>
        <Slider hasTrack={ false } dataSplide={`{"type":"loop", "pagination": false, "start": ${props.currentImage - 1}}`} moved={movedHandler}>
          <SplideTrack>
            {sliderContent}
          </SplideTrack>
